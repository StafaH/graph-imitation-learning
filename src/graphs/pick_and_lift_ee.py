...
Examples 
    train with mlp::
        $ python src/graphs/pick_and_lift_ee.py --tag ee_64x3 --seed 53 --hidden_dims 64 64 64 --data_dir data/pick_and_lift/ --model_name mlp --num_epochs 1000

    test mlp::
        $ python src/graphs/pick_and_lift_ee.py --eval --eval_batch_size 10 --max_episode_length 250 --checkpoint_dir logs/pl_mlp_feat_64x3/seed53_{...} --seed 53
...
def obs_to_input(obs, use_relative_position=True):
    ...
    gripper_pose = np.concatenate([obs.gripper_pose, GRIPPER_ENC, [obs.gripper_open]])
    features.append(gripper_pose)  # (10,) +1 for gripper_open
    ...
    feature = np.concatenate(features)  # (41,)
    return feature
...
def load_data(data_dir):
    ...
    for d in episode_dirs:
        ...
        for t in range(len(data._observations) - 1):
            ...
            full_action = np.concatenate([delta, [data._observations[t + 1].gripper_open]])
            y = torch.tensor([full_action],
                             dtype=torch.float)
