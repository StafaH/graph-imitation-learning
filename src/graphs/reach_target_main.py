...
Examples 
    train with mlp::
        $ python src/graphs/reach_target_main.py --tag vel_hid64-64-64 --seed 2 --hidden_dims 64 64 64 --data_dir data/reach_target/ --model_name mlp --num_epochs 1000

    test mlp::
        $ python src/graphs/reach_target_main.py --eval --eval_batch_size 10 --max_episode_length 100 --checkpoint_dir logs/rt_mlp_vel_hid64-64-64/seed2_{...}/ --seed 6

    train with graph:: 
        $ python src/graphs/reach_target_main.py --tag vel_hid64-64-64 --seed 2 --hidden_dims 64 64 64 --data_dir data/reach_target/ --model_name graph --num_epochs 1000
    
    test graph:: 
        $ python src/graphs/reach_target_main.py --eval --eval_batch_size 10 --max_episode_length 100 --checkpoint_dir logs/rt_graph_vel_hid64-64-64/seed2_{...}/ --seed 6
...
def make_graph_dataset(data_dir, relative_pos=False, joint_vel=True):
    ...
    for d in episode_dirs:
        ...
        for t in range(len(data._observations) - 1):
            ...
            if joint_vel:
                y = torch.tensor([data._observations[t].joint_velocities],
                                 dtype=torch.float)
            else:
